    ...
    for row in range(len(grid)):
        for col in range(len(grid[0])):
            ...
            for path in surr_paths([row, col]):
                # is_island = True
                if not is_valid_cell(path, grid):
                    continue
                elif grid[path[0]][path[1]] == 1:
                    is_island = False
                    break  # Optimization: if one neighbor is 1, it's not an island
            if is_island:
...
if __name__ == '__main__':
    print(
        "Number of islands identified in the grid are %i" % no_of_islands(grid)
    )